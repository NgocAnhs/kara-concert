...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [x]`) syntax for tracking. The Sites-owning agent edits site source; independent reviewers may inspect without edits.
...
- [x] Check hosting with the existing project ID; do not create a replacement if access fails. Current read returned `project_not_found`; local implementation can continue, publication needs correct access.
...
- [x] Re-run targeted tests; run `npm run build`; start retained Vite server, make one HTTP request to its printed URL, then open that URL using the existing Site preview tab if available. No screenshots/DOM inspection.
- [x] If social preview is missing, dispatch exactly one image-only agent after first preview, per Sites. It must not edit source, call Sites, or spawn agents. Owner integrates only a verified usable image.
...
- [x] Use requesting-code-review for an independent read-only review; owner resolves in-scope findings using TDD. Never let reviewer edit the Site checkout or call Sites.
- [ ] Publish after the existing Site access is restored. `project_not_found` blocks source push/version saving/deployment and absolute social-image metadata; no replacement Site or access change is authorized. Validated source is committed locally and the preview remains available.
...
- Existing Sites ID `appgprj_6a8d0853184081919eb80e16fe50311b` returned `project_not_found`. No replacement project, push, deployment or access modification was attempted.
