...
export async function listPublishedSongs(client: PublicCatalogClient): Promise<Song[]> {
  const { data, error } = await client
    .from('songs')
    .select('id,title,youtube_url,source,ai_model,prompt_version,lyric_lines(id,korean,viet_han,romanization,meaning,display_order,start_seconds,end_seconds)')
    .order('title')
