...
where n.nspname = 'public' and c.relname in ('import_jobs', 'import_attempts', 'access_attempts', 'youtube_metadata', 'import_runtime', 'gemini_outputs');
select is((select count(*) from pg_policies where schemaname = 'public'
  and tablename in ('import_jobs', 'import_attempts', 'access_attempts', 'youtube_metadata', 'import_runtime', 'gemini_outputs')), 0::bigint,
...
from unnest(array['anon', 'authenticated']) role_name
cross join unnest(array['import_jobs', 'import_attempts', 'access_attempts', 'youtube_metadata', 'import_runtime', 'gemini_outputs']) table_name
cross join lateral (values
  ...
  ('update', format('update public.%I set %I = %I', table_name,
    case when table_name = 'youtube_metadata' then 'video_id' when table_name = 'access_attempts' then 'ip_hash'
      when table_name in ('import_attempts', 'gemini_outputs') then 'job_id' else 'id' end,
    case when table_name = 'youtube_metadata' then 'video_id' when table_name = 'access_attempts' then 'ip_hash'
      when table_name in ('import_attempts', 'gemini_outputs') then 'job_id' else 'id' end)),
...
select ok(not has_table_privilege('service_role', 'public.gemini_outputs', 'UPDATE,TRUNCATE'),
  'backend cannot alter or truncate captured Gemini responses');
